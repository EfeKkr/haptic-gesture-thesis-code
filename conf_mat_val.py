# Generates normalized confusion matrices on the validation set
# for CNN+LSTM and ViT+LSTM models (100%, 60%, 30%).
# Results are saved in norm_cmatrices_val/.
...
os.makedirs("norm_cmatrices_val", exist_ok=True)
...
for cfg in models:
    ...
    plt.title(f"{cfg['name']} - Normalized Confusion Matrix (Validation Set)")
    plt.tight_layout()

    fname = cfg["name"].replace(" ", "_").replace("+", "plus").replace("(", "").replace(")", "")
    save_path = os.path.join("normalized_confusion_matrices", f"{fname}.png")
    plt.savefig(save_path)
    plt.close()

print("All normalized confusion matrices saved.")