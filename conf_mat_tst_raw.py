# Generates raw (non-normalized) confusion matrices on the test set
# for all CNN+LSTM and ViT+LSTM models (100%, 60%, 30% video length).
# Uses dyad-based splits and saves plots to raw_cmatrices_test/.
...
os.makedirs("raw_cmatrices_test", exist_ok=True)

for cfg in models:
    print(f"Generating raw confusion matrix for {cfg['name']}")
    ...
    # Compute raw confusion matrix (non-normalized)
    cm = confusion_matrix(all_labels, all_preds)

    fig, ax = plt.subplots(figsize=(10, 8))
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=class_names)
    disp.plot(ax=ax, xticks_rotation=45, cmap="Blues", values_format="d", colorbar=True)
    plt.title(f"{cfg['name']} - Raw Confusion Matrix (Test Set)")
    plt.tight_layout()

    fname = cfg["name"].replace(" ", "_").replace("+", "plus").replace("(", "").replace(")", "")
    save_path = os.path.join("raw_confusion_matrices_test", f"{fname}.png")
    plt.savefig(save_path)
    plt.close()

print("All raw confusion matrices (test set) saved.")